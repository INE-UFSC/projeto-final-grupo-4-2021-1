...
class LevelUpState(BaseMenuState):
    def __init__(self):
        ...
        self.active_index = 0
        self.__title = Text("versao_final/assets/fonts/menu_option.ttf", 80, pygame.Color(255, 255, 255), "LEVEL UP!")
        self.__availablePoints = Text("versao_final/assets/fonts/menu_option.ttf", 50, pygame.Color(255, 255, 255), "Available Points: " + str(MainCharacter().stats.availablePoints))
        ...
        self.options: List["TextPressable"] = [TextPressable("versao_final/assets/fonts/menu_option.ttf", 50, text, (100 + 300*index, self.screen_rect.height/2 )) for index, text in enumerate(MainCharacter().stats.stats)]
        self.stats = [Text("versao_final/assets/fonts/menu_option.ttf", 50, pygame.Color(255, 255, 255), str(MainCharacter().stats.stats[option.text])) for option in self.options]
